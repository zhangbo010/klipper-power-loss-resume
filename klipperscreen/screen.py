...
class KlipperScreen(Gtk.Window):
    ...
    def init_usb_monitor(self):
        try:
            ...
            logging.warning("pyudev is not installed. USB device listening is disabled.")
            return
        context = pyudev.Context()
        monitor = pyudev.Monitor.from_netlink(context)
        monitor.filter_by(subsystem='block')
        observer = pyudev.MonitorObserver(monitor, callback=self.usb_event, name='usb-monitor')
        observer.start()
        logging.info("USB device listening is enabled.")
    def usb_event(self, device):
        if device is None or not hasattr(device, 'properties'):
            return
        props = device.properties
        if (
            props.get('DEVTYPE') == 'disk' and
            props.get("ID_BUS") == "usb" and
            props.get("ID_USB_DRIVER") == "usb-storage"
        ):
            ...
            if action == "add":
                logging.info(f"USB device {devname} added.")
                GLib.idle_add(self.show_popup_message, _("USB drive %s is inserted") % model, 1, False)
            elif action == "remove":
                logging.info(f"USB device {devname} removed.")
                GLib.idle_add(self.show_popup_message, _("USB drive %s is removed") % model, 2, False)
